...
public class BloomFilter<T> {
   ...
   BloomFilter( final int size ) {
      ...
      hashFunctions = Arrays.asList( multiplyWith( 29 ),
                                     multiplyWith( 19 ),
                                     multiplyWith( 17 ) );
   }
   ...
   public void add( final T value ) {
      hashFunctions.stream().map( function -> function.apply(
               value ) ).forEach( this::mark );
   }

   public boolean exists( final T value ) {
      return hashFunctions.stream().map( function -> function.apply(
               value ) ).allMatch( this::isMarked );
   }
...
class BloomFilterMain {
   public static void main( String[] args ) {
      ...
      // This should be false, but bloom filters provide no guarantee
      // on definite existence.
