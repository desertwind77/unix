...
class CreditCard:
    def __init__( self, name, filename, extension, has_negative_amount, has_header, fields ):
        self.name = name
...
class Statement:
    def __init__( self, filename, credit_card, category_info ):
        self.filename = filename
    ...
    def get_category( self, description ):
        ...
        for merchant, category in self.category_info.items():
            if re.search( merchant.lower(), description ):
                known_category = category
                break
        return known_category

    def load_transactions( self, category_info ):
        '''Read and categorize all transactions in the statement'''
        ...
        with open( self.filename, 'rt', encoding='utf-8' ) as file:
            ...
            for row in reader:
                ...
                description = row[ field_desc ].lower()
...
class CreditReport:
    def __init__( self, config_filename, statement_location ):
        ...
        self.statements = []
        self.expense_by_category = {}
        #self.monthly_expenses = { i : MonthlyExpenses( i ) for i in range( 1, 13 ) }
    ...
    def load_credit_card_config( self ):
        ...
        for name, info in credit_card_config.items():
            ...
            fields = info[ 'Fields' ]
            self.creditcard_info[ name ] = CreditCard( name, filename, extension,
                                                       has_negative_amount,
                                                       has_header, fields )
    ...
    def generate_statements_from_files( self ):
        ...
        for statement_file in statement_path.glob( '*.csv' ):
            filename = statement_file.stem

            statement = None
            for _, card in self.creditcard_info.items():
                obj = re.match( card.filename, filename )
                if obj:
                    statement = Statement( str( statement_file ), card, self.category_info )
            assert statement is not None, f'Unable to find the credit card for {statement_file.name}'
    ...
    def load_all_transactions( self ):
        ...
        for statement in self.statements:
            statement.load_transactions( self.category_info )
        ...
        if unknown_transactions:
            for transaction in unknown_transactions:
                print( Path( transaction.statement.filename ).name, transaction.description )
